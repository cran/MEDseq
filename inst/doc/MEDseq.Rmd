...
### Installing MEDseq
__MEDseq__ will run in Windows, Mac OS X, or Linux. To install it you first need to install [R](https://cran.r-project.org/). Installing [RStudio](https://rstudio.com/) as a nice desktop environment for using R is also recommended.
...
Other plot types adapted from __TraMineR__ can be produced using `type="d"` (state distribution plots), `type="f"` (state frequency plots), `type="i"` (selected sequence index plots), `type="I"` (whole set index plots), `type="ms"` (modal state plots; an alternative to `type="mean"` above), and `type="mt"` (mean times plots). Each of these plots are shown on a per-cluster basis, and all account for the sampling weights (if any). Clustering uncertainties, the gating network, and model selection criteria can also be visualised. 
